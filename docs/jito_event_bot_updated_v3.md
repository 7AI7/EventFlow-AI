...
**Workflow**:
1. Query: Users with events 1-3 days away
2. For each user: AI generates themed content from event description
3. Send WhatsApp with content
4. Rate limit: 10 msgs/sec
...
**Triggers**: 11:30 AM, 3 PM, 6:30 PM

**Workflow**:
1. Query: Events happening now (±1 hour)
2. Fetch food locations for those events
3. Query: Attendees of current events
4. Loop: Send food announcement with sponsor
...
**Query**:
```sql
SELECT u.phone, u.name, e.name, e.time, e.location, e.gmaps_link, e.contact_person, e.contact_phone
FROM users u
JOIN registrations r ON u.phone = r.phone
JOIN events e ON r.event_id = e.id
...
**Message**: "Good morning {name}! {event} today at {time}. Location: {location}. Map: {gmaps_link}"
...
**Query**:
```sql
SELECT u.phone, u.name, e.name, e.time, e.location
FROM users u
JOIN registrations r ON u.phone = r.phone
JOIN events e ON r.event_id = e.id
WHERE e.date = CURRENT_DATE 
  AND e.time BETWEEN NOW()::time + INTERVAL '55 minutes' 
                 AND NOW()::time + INTERVAL '65 minutes'
  AND e.cancelled = false;
```

**Message**: "⏰ {name}, {event} starts in 1 hour! Location: {location}"
